...
## [2026-06-15] update | Corporate flow — planning docs from Samuel's brief
- Samuel (corporate bookings) flagged Cassie giving wrong corporate info in production + briefed a new two-flow design (individual vs corporate)
- Mapped the corporate-enquiry surface: corporate answers come from corporate-funding.md, corporate-invoicing.md, faq/funding.md, faq/registration.md, concepts/wsq-registration-data.md (corporate section), plus shared ssg-subsidies.md, pricing-tiers.md, company/overview.md, company/contact.md
- Identified likely wrong-info root cause: SME subsidy conflict — ssg-subsidies.md says SME=70%, but corporate-funding.md + faq/funding.md say ETSS "up to 90%". Cassie can quote either.
- Created 3 review docs at repo root: `corporate-flow-plan.md` (flow design + blast radius + phasing + answers Mark's Zoho-custom-module question), `corporate-kb-review-for-samuel.md` (full text of every corporate KB source for Samuel to comment on), `corporate-table-proposal-for-samuel.md` (proposed corporate_enquiries field spec)
- Decisions captured from Mark (2026-06-15): deduce flow / ask only if ambiguous; answer-first then collect; corporate form already exists on site. OPEN: data destination (Cassie admin table vs Zoho custom module vs both — rec A now→C later).
- No code or persona changes yet — Phase 0 (Samuel content review) is the gate before building.
- Follow-up (same day): Mark answered the open questions. Data destination = Option A (cassie_db table + /admin/corporate page). Corporate form CONFIRMED at /corporate-enquiry/ (vault's /corporate-training/ URL is wrong — fix pending), accepts URL params (course_name, tag, deal_trigger, corporate_deal=Yes, todayDate), creates Zoho Contact; attribution via trafficSource not utm_chat. No min group size (route by buyer, not headcount). No provider field needed. SME-figure-quoting + corporate-pricing-source parked until Samuel reviews the KB doc. Updated corporate-flow-plan.md (§1a, §2, §3, §6) + corporate-kb-review-for-samuel.md (URL correction).
- Chrome exploration of live corporate pages (2026-06-15): /corporate-training/ = info/marketing landing + simple enquiry form + funding overview (incl. Enterprise Innovation Scheme, missing from vault) + customised(non-funded)-vs-contextualised(funded) rule. /corporate-enquiry/ = the real registration funnel: "3 steps" (Book→Pay&Attend→Claim AP via EPJS ~2.5wks), form captures rep name/org/UEN/email/phone/SME, emails bulk registration form. **ROOT CAUSE of wrong corp info FOUND:** ETSS page shows up-to-90% applies ONLY to CET Centre/university/poly/ITE courses; certifiable courses by EXTERNAL training providers (= Coursemology) get UP TO 70%. Vault's "ETSS up to 90%" in corporate-funding.md + faq/funding.md is WRONG for Coursemology → should be 70%. AP $4.50/hr + $100k cap MATCHES vault (correct). Also missing from vault: EIS scheme, 2nd AP tier (95% for SC≥35/low-income), 3-step process, EPJS SME-status check. Updated all 3 corporate review docs with these findings.
- Samuel review returned (corporate-kb-review-for-samuel-updated-with-comments.docx). Verdicts: (1) ETSS = up to 70% for Coursemology CONFIRMED (90% only for other provider types) → fix corporate-funding.md + faq/funding.md; (2) Corporate/sales WhatsApp = 8899 1945 NOT 9866 0772 (flagged 3x) → fix corporate-funding.md, corporate-invoicing.md, company/contact.md corporate sections; (3) AP & ETSS = Singaporeans/PR only; (4) ADD EIS scheme = 20% of remaining course fees after subsidies & SFEC; skip 2nd AP tier (not needed); (5) SME-sponsored pricing: PR gets 70% (not 50%) + all locals get 70% regardless of age; (6) credit terms → route to corporate training consultant, don't quote; (7) in-chat capture + separate DB ENDORSED ("capture all potential leads"); still do manual TP-Gateway submission after. OPEN/CONTRADICTION: Samuel's B2 GST comment ("discount applied to pre-subsidy fee so GST lower") conflicts with vault's "GST on full pre-subsidy fee" individual rule — flagged for Samuel, NOT auto-applied. No vault edits made yet pending Mark's go-ahead.

## [2026-06-15] update | Corporate KB corrections applied (Samuel's review)
- corporate-funding.md: ETSS 90%→**70%** (+ note 90% only for CET Centre/uni/poly/ITE/NIEC, not us); AP & ETSS marked **SC/PR only**; SME definition (≤200 staff or ≤$100M turnover) + EPJS SME-status check added; **added Enterprise Innovation Scheme (EIS) = 20% of remaining fees after subsidies & SFEC**; "How to Book" now points to corporate-enquiry form + **corporate sales WhatsApp 8899 1945** (was 9866 0772)
- faq/funding.md: "company wants to sponsor me" answer 90%→70% + SC/PR-only + SFEC/EIS mention
- ssg-subsidies.md: added SME-sponsored rates note (PR gets 70% not 50%; all SC 70% regardless of age; 70% is our ceiling)
- faq/registration.md: corporate-booking answer now routes to corporate-enquiry form / in-chat capture + corporate WhatsApp 8899 1945
- company/contact.md: corporate section → corporate-enquiry form + corporate sales WhatsApp 8899 1945 (general 9866 0772 retained for individual)
- corporate-invoicing.md: credit-terms → "contact corporate training consultant"; both WhatsApp refs 9866 0772→8899 1945
- index.md: corporate-funding summary updated (ETSS up to 70%, + EIS)
- **NOT changed (pending Samuel):** GST mechanics. Vault says "GST on full pre-subsidy fee / computed before discounts"; Samuel's note says discount lowers GST. Quote pair given to Mark to confirm with Samuel by text.
- last_updated bumped to 2026-06-15 on all 5 edited content files
- Drafted corporate-persona-section-draft.md (repo root): the "Corporate Flow" behaviour section (Corporate Rules 1-6 + worked example) to slot into cassie-persona.md LATER. Held separate on purpose — it depends on an unbuilt server tool save_corporate_enquiry (writes corporate_enquiries row, returns [[CORP_BOOK]] token) + table + /admin/corporate page + token substitution. Keeping it out of the live persona lets Mark push the corrected corporate KB facts now without shipping a half-wired flow. Architecture explained to Mark: vault=facts, persona=behaviour, server=machinery(+tool).

## [2026-06-15] update | Corporate Flow section MERGED into cassie-persona.md
- Mark gave the go-ahead to edit the live persona (he'll hold the push until vault + deploy repo are all in place, so persona + server land together).
- Revised the What-you-CANNOT-do corporate line: was "Handle bulk corporate bookings... Refer to the team" → now "Complete a corporate/group registration yourself (corporate team does that), but you DO handle corporate enquiries: answer, capture, hand off. See Corporate Flow."
- Appended the **Corporate Flow** section (Corporate Rules 1-6 + worked example) at the end of the persona, em-dash-free in Cassie's quoted lines.
- Bumped persona frontmatter last_updated with a 2026-06-15 note flagging the save_corporate_enquiry tool + [[CORP_BOOK]] token dependency.
- Forminator prefill verified earlier: 6/7 fields prefill (course_name, rep_name, org_name, uen, contact_email, contact_phone); SME select won't prefill (Forminator quirk, config is correct) — left as-is, non-blocking.
- DEPLOY GATE: persona now references save_corporate_enquiry + [[CORP_BOOK]] which do NOT exist server-side yet. Do NOT push until the corporate_enquiries table + save_corporate_enquiry tool + [[CORP_BOOK]] substitution + /admin/corporate page are built.

## [2026-06-15] update | Corporate flow SERVER BUILD complete (cassie-deploy)
- DEPLOY GATE NOW CLEARED — all four server pieces built and verified (py_compile + standalone logic test all pass):
- `sql/init.sql`: added `corporate_enquiries` CREATE TABLE (fresh installs).
- `app/db_logger.py`: added `CORPORATE_TABLE_DDL`, `ensure_corporate_table()`, and synchronous `upsert_corporate_enquiry(...)` (own per-call autocommit connection; CREATE TABLE IF NOT EXISTS self-heals existing DBs; UPSERT keyed on salesiq_chat_id, COALESCE keeps prior values on enrich, status never reset).
- `app/server.py`: `build_corporate_url()` (pre-fills the 6 confirmed Forminator params + corporate_deal=Yes + todayDate + trafficSource=Cassie; SME omitted); `execute_save_corporate_enquiry()` (upserts row, returns [[CORP_BOOK]] token + url + a "missing phone" nudge); registered `save_corporate_enquiry` as 2nd entry in CASSIE_TOOLS; `CORP_TOKEN_RE` + `_substitute_corporate_token()`; loop dispatch elif + token→URL mapping (strips raw url like booking_url) + substitution after the URL scrub (so the link survives); `ensure_corporate_table()` best-effort at boot.
- `app/admin_routes.py`: new `/admin/corporate` page (CORPORATE_TEMPLATE + list_corporate route) — worklist with phone surfaced prominently (red "no phone" when missing), status/date/search filters, rows link to the chat transcript; graceful empty-state if table doesn't exist yet (catches err 1146); added cross-links between conversations/dashboard/corporate.
- Verified: BOOKING_TOKEN_RE does NOT match [[CORP_BOOK]] (no collision); token survives _scrub_urls then resolves to [Complete your enquiry](url); unmapped token stripped silently; existing booking flow untouched.
- contained_booking_link metric left as booking-only (corporate handoff NOT counted as a booking link, preserving link_rate meaning); corporate volume tracked via the corporate_enquiries table instead.
- READY TO DEPLOY: push cassie-vault (KB fixes + persona Corporate Flow) + cassie-deploy together; friend still to confirm the trafficSource attribution param; Samuel's GST clarification still pending (KB GST text untouched).

## [2026-06-17] update | GST rule corrected (discount lowers GST) + SME/non-SME tiers
- GST FINAL RULE (Mark confirmed w/ his dad, 2 screenshots): GST (9%) is on the **net fee after any DISCOUNT, before SUBSIDY**. A discount comes off first and LOWERS the GST; a subsidy lowers the fee payable but NOT the GST. (Correction: my prior same-day note wrongly said GST is always on the original price / no edit needed — Samuel's review comment "discount applied to pre-subsidy fee so GST lower" was actually CORRECT.)
  - No-discount: full 150 → GST 13.50 → SC40+ 45+13.50=58.50, PR 75+13.50=88.50.
  - With $30 discount: full 150 − 30 = net 120 → GST 10.80 (on net 120).
  - Edited: pricing-tiers.md (rule rewritten + discount Example B added), ssg-subsidies.md (GST section), corporate-invoicing.md ("before any discounts" → "after discount, before subsidy").
- SME vs NON-SME tiers (Samuel): SME (ETSS) = flat **70% by citizenship, NO age bands** (every SC any age + PR). Non-SME / MNC / corporate entity = NO ETSS → standard INDIVIDUAL tiers (SC 21-39=50%, SC 40+=70% MCES, PR=50%).
  - Edited: ssg-subsidies.md Company-Sponsored table (was flat "Non-SME 50%" → now individual tiers + notes), corporate-funding.md (added Non-SME section + "ETSS 70% by citizenship, no age" note), cassie-persona.md Corporate Rule 3 (only SMEs get flat 70%; non-SME = individual rates; don't guess if unsure).

## [2026-06-17] note | Local-test guide + corporate-flow visual created
- Confirmed cassie-deploy is Docker-ready for local testing: corporate code adds NO new deps (pymysql + stdlib only); corporate_enquiries auto-created (init.sql on fresh DB, ensure_corporate_table() at boot otherwise).
- Created `cassie-deploy/LOCAL_TESTING_GUIDE.md` — Docker Desktop (Windows/PowerShell) walkthrough: .env setup, `docker compose up -d --build`, verify table, test corporate flow via /chat (PowerShell Invoke-RestMethod), check [[CORP_BOOK]] link resolves, view /admin/corporate, teardown, troubleshooting.
- Created `corporate-flow-visual.html` (repo root) — printable single-file visual: individual-vs-corporate split, detection signals, 5-stage flow, annotated example chat, captures-vs-never, data destinations (table+link→Zoho), corrected funding cheat-sheet (70% SME no-age, non-SME individual tiers, AP, EIS, GST-on-net-after-discount, WhatsApp 8899 1945).
- No code/vault/persona changes this step — both are deliverables for Mark's local testing.

## [2026-06-18] update | First corporate-flow test run reviewed — 9/11 clean, 2 fixes
- Mark ran corporate_flow_test.py against local Docker (corporate_test_results_20260617_2350.md). Result: 9 of 11 scenarios correct (S1 happy-path, S2 phone-chase, S3 routing, S4 SME-70%-not-90%, S5 non-SME individual tiers, S7 individual regression, S8 GST net-after-discount, S9 on-site, S10 declines-phone-graceful all PASS).
- BUG 1 (S11): "What's the Enterprise Innovation Scheme" was BLOCKED by the classifier (returned the off_topic deflection), not answered. Root cause: CLASSIFIER_SYSTEM allow-list named SFC/PSEA/UTAP/MCES but not corporate schemes. FIX (app/server.py): added a bullet allowing ETSS/AP/SFEC/EIS/SME + an ALLOW example. Needs rebuild.
- BUG 2 (S6): individual "[Book this class]" link leaked into a corporate chat (Cassie recognised corporate but still emitted the [[BOOK_]] token from get_course_schedule). Root cause: Booking Links "always attach a token" rule overpowered Corporate Rule 2. FIX (cassie-persona.md): added a "Corporate exception (read first)" callout at the top of Booking Links + strengthened Corporate Rule 2 ("NEVER ... even though the tool returns one", marked as overriding Booking Links, added a dates-without-link example).
- Minor (not fixed, acceptable): S10 quoted individual price tiers in a corporate chat; S2 link course_name carried the "WSQ" prefix the user typed. Both harmless.
- Demo observation from Mark ("said 'food hygiene', Cassie didn't ask corp/indiv") = CORRECT BY DESIGN: a plain course query with no corporate signal defaults to individual; the routing question is only for ambiguous-group cases.
- NEXT: Mark rebuilds (`docker compose up -d --build` — picks up classifier code + re-reads mounted persona + resets the rate limiter), re-runs the test, pastes results; focus on S6 + S11.

## [2026-06-18] update | Re-test: S11 fixed, S6 needed a server-side guard
- Re-run (corporate_test_results_20260618_0029.md): S11 (EIS) NOW PASSES — classifier fix worked, Cassie explains EIS (20% after SSG+SFEC) + defers eligibility. S1-S5, S7-S10 still good.
- S6 STILL leaked the individual [[BOOK_]] link in a corporate chat even after the persona rebuild — Haiku keeps attaching the schedule tool's token despite Corporate Rule 2. Persona-only suppression proven unreliable.
- FIX (app/server.py) — server-side safety net: added `corporate_convs` set + `_CORP_SIGNAL_RE` (employer signals: my/our staff|team|company, employees, UEN, SME, MNC, ETSS/SFEC/EIS, absentee payroll, corporate, on-site, "N staff", etc.). Conversation is flagged corporate when a user turn matches OR when save_corporate_enquiry is called (sticky). `_substitute_booking_tokens` gained a `suppress` param; in corporate convos ALL [[BOOK_]] tokens are stripped (not rendered). Standalone-tested: corporate phrases match, individual phrases ("for myself", "food hygiene", "excel courses") don't; suppress strips the link, leaves the date text. Persona changes kept too (defense in depth).
- NOTE: bash py_compile gave a false "unterminated string at line 1658" — the sandbox mount served a TRUNCATED copy of server.py (the error is in the untouched /logs docstring). Real file intact (Read-tool verified the new regex block; earlier full py_compile passed). Docker build is the authoritative compile.
- NEXT: Mark rebuilds (`docker compose up -d --build`), re-runs test, pastes results; verify S6 now shows July dates with NO individual booking link (look for `indiv_link` flag GONE on S6). If container fails to start, paste logs.

## [2026-06-18] update | S6 FIXED — all 11 corporate scenarios PASS
- Re-test of S6 (corporate_test_results_20260618_0041.md, `--only S6`): Cassie listed "1 July at Jurong East" as plain text, NO booking link, signals: (none) — indiv_link flag GONE. She also explained she's routing to the corporate team instead of individual links. Server-side guard confirmed working.
- **ALL 11 corporate scenarios now PASS** (S1-S11). Corporate flow fully validated locally. server.py compiles in Docker (container running). 
- READY TO DEPLOY when Mark chooses. Deploy files (cassie-deploy): app/server.py, app/db_logger.py, app/admin_routes.py, sql/init.sql, .gitignore (explicit git add to skip CRLF churn). Vault repo: persona + funding/faq/company KB edits. Order: push vault → push cassie-deploy → Weixing `git pull && docker compose up -d --build`. No env/deps/migration.

## [2026-06-18] DEPLOY | Corporate flow shipped to production
- Mark pushed vault (persona Corporate Flow + corrected corporate KB facts) + cassie-deploy (corporate_enquiries table, save_corporate_enquiry tool, pre-filled [[CORP_BOOK]] link, /admin/corporate page, classifier corporate-scheme allow-list, server-side individual-link suppression guard). Weixing rebuilt. LIVE.
- Capture is intent-gated (row written when Cassie calls save_corporate_enquiry, i.e., once enough contact/intent gathered — not every corporate convo) — Mark confirmed this is the desired behaviour.
- Also done this period (separate from corporate): SalesIQ pre-chat form gate REMOVED (Email/Phone online fields) after dashboard analysis showed ~61% convo drop since it went live; watching volume recover (~check 2026-06-25); individual in-chat lead capture planned but ON HOLD ~1 week.
- Chapter closed.

## [2026-06-18] fix | Two post-deploy fixes from Mark's live SalesIQ test
- Mark tested deployed corporate flow via SalesIQ. Logs confirmed server + table 100% correct (save_corporate_enquiry got the right email weeingyau@gmail.com, built the full correct URL with all params). BUT the link DISPLAYED in SalesIQ was mangled: contact_email=weeingyau%20for%20your%20SME%20team, missing the trailing params. Root cause = SalesIQ's chat renderer/auto-linker chokes on the "@" in the email inside the URL (sees an email, breaks the link there, pulls in adjacent reply text "for your SME team"). Same SalesIQ link-mangling class as the old booking-link bug; individual booking URLs are unaffected because they have no "@".
- FIX A (app/server.py build_corporate_url): OMIT contact_email from the pre-filled URL (no "@" → SalesIQ renders cleanly). Email still saved to corporate_enquiries; the form requires email so the visitor types it there. py_compile OK.
- FIX B (cassie-persona.md Corporate Rule 5): Cassie was asking one field per turn (phone→name→company→UEN→email) = tedious. Rewrote Rule 5 to ask for the whole set in ONE message (name, company+UEN, email, SME, phone), phone still the must-have, with an example.
- REDEPLOY: push cassie-persona.md (vault) + app/server.py (cassie-deploy); Weixing `git pull && docker compose up -d --build`. Then RE-TEST in SalesIQ: confirm link renders fully (no "@", clean) + Cassie asks in one shot. If link still mangles after removing email, escalate (length?), but "@" is the strong suspect.
